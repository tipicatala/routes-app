      ...
      {resultData ? (
        <div>
          ...
          <List>
            ...
            {queryParams.changes && (
              <ListItem>
                <ListItemText primary={`Change Cities: ${queryParams.changes}`} />
              </ListItem>
            )}
            <ListItem>
            ...
            <ListItem>
              <ListItemText primary={`Number of Passengers: ${queryParams.passengers}`} />
            </ListItem>
          </List>
          ...
          <List>
            {resultData.distances.map((distance, index) => (
              <ListItem key={index}>
                <ListItemText
                  primary={`Distance from ${distance.from} to ${distance.to}: ${distance.distance.toFixed(2)} km`}
                />
              </ListItem>
            ))}
            <ListItem>
              <ListItemText primary={`Total Distance: ${resultData.totalDistance.toFixed(2)} km`} />
            </ListItem>
          </List>
        </div>
      ) : (
        <Typography variant="body1">An error occurred while calculating distances.</Typography>
      )}
    </Container>
  );
};
